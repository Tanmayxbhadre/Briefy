        ...
        if (item.drafts && item.drafts.length > 0) {
          ...
          await prisma.articleDraft.update({
            ...
            data: {
              ...
              content: draft.content?.trim() || `## What Happened\n\n${item.description || item.title}\n\n## Key Details\n\nOriginal reporting and verification provided by ${item.source.name}.\n\n## Why It Matters\n\nFollow BRIEFY for ongoing editorial updates and verified global coverage.`,
              excerpt: draft.excerpt?.trim() || item.description || item.title,
              authorName: draft.authorName || session.user || 'BRIEFY Editorial Desk',
              seoTitle: draft.seoTitle || `${item.title} — BRIEFY`,
          ...
          await prisma.articleDraft.create({
            data: {
              ...
              authorName: session.user || 'BRIEFY Editorial Desk',
              featuredImage: item.imageUrl || '',
              imageAlt: item.imageAlt || item.title,
              status: 'PUBLISHED',
              publishedAt: item.publishedAt || new Date(),
              seoTitle: `${item.title} — BRIEFY`,
